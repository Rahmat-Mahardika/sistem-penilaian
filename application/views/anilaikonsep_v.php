	...
	<div class="panel panel-default">    	
        <div class="panel-heading">
        	...
        	<table class="table table-hover table-striped table-condensed" id="table">
                ...
                <tbody>                
                <?PHP
                	$query = $this->apenilaian_m->viewallnilaikonsep();
					$no = 1;
				...
				?>
                    <tr>
                        ...
                        <td><?PHP echo $row->keterangan; ?>
                            <input type="hidden" id="idnilaikonsep_<?php echo $row->idnilaikonsep; ?>" value="<?php echo $row->idnilaikonsep; ?>" />
                            <input type="hidden" id="noinduk_<?php echo $row->idnilaikonsep; ?>" value="<?php echo $row->noinduk; ?>" />
                            <input type="hidden" id="penilaian_<?php echo $row->idnilaikonsep; ?>" value="<?php echo $row->idpenilaian; ?>" />
                            <input type="hidden" id="nilai_<?php echo $row->idnilaikonsep; ?>" value="<?php echo $row->nilai; ?>" />
                            <input type="hidden" id="keterangan_<?php echo $row->idnilaikonsep; ?>" value="<?php echo $row->keterangan; ?>" />
                        </td>
                        ...
                        <td>
                            <button class="btn btn-warning btn-sm edit" title="Ubah" data-toggle="modal" data-target="#modal_penilaian" id="edit_<?PHP echo $row->idnilaikonsep; ?>">
                                <i class="glyphicon glyphicon-edit"></i> Ubah
                            </button>
                            <button class="btn btn-danger btn-sm delete" title="Hapus" data-toggle="modal" data-target="#modal_confirm" id="delete_<?PHP echo $row->idnilaikonsep; ?>" >
            ...
            <div class="modal-body">
            	<form method="post" id="form_penilaian">
                	<div class="form-group">
                        ...
                        <input type="hidden" name="idnilaikonsep_tmp" id="idnilaikonsep_tmp" />
                	...
                	<div class="form-group">
						...
						<?php
						$query = $this->apenilaian_m->viewnilaikonsep();
						foreach($query->result() as $row) :
						?>
...
<script type="text/javascript">
	$(document).ready(function(){
		$('.add').click(function(){
			...
			$('#form_penilaian').attr('action','<?PHP echo base_url(); ?>anilaikonsep/save');
		...
		$(".edit").click(function(){
			...
			$('#form_penilaian').attr('action','<?PHP echo base_url(); ?>anilaikonsep/update');
		...
		$("#delete").click(function() {
			$.post("<?PHP echo base_url(); ?>anilaikonsep/delete", { 
				idnilaikonsep: $("#idnilaikonsep_tmp").val() }, 
				function() {
					window.location = "<?PHP echo base_url(); ?>anilaikonsep";
				}
			);
//			 	window.location = "<PHP echo base_url(); ?>program_keahlian/" + $("#id_program_keahlian").val();
		...
		$("#delete_all").click(function() {
			$.post("<?PHP echo base_url(); ?>anilaikonsep/delete_all", {}, 
				function() {
					window.location = "<?PHP echo base_url(); ?>anilaikonsep";
				}
			);
		...
		$('.excel').click(function() {
			window.location = '<?PHP echo base_url(); ?>anilaikonsep/export/excel';
		});

		$('.pdf').click(function() {
			window.location = '<?PHP echo base_url(); ?>anilaikonsep/export/pdf';
		});
